  ...
  for (let match of matches) {
      ...
      if (opMatch) {
         ...
         if (opEndIndex !== -1) {
             ...
             if (originalOp !== 'null') {
                // If it looks like a function that can be synced
                // Sometimes onPressed is assigned a reference like 'null' or it's a valid closure
                let safeOp = `loadingState.isLoading ? null : () => loadingState.runAsyncAction(() async { await Future.sync(${originalOp}); })`;
