  ...
  Future<String> _extractNoteContent() async {
    try {
      if (_uploadedBytes != null && _uploadedFileName != null) {
        final fileName = _uploadedFileName!.toLowerCase();
        if (fileName.endsWith('.txt')) {
          return String.fromCharCodes(_uploadedBytes!);
        }
        // For PDF and DOCX, you would need additional parsing libraries
        // For now, return empty string for non-text files
        return '';
      } else if (_uploadedFile != null) {
        final fileName = _uploadedFile!.path.toLowerCase();
        if (fileName.endsWith('.txt')) {
          return await _uploadedFile!.readAsString();
        }
        return '';
      }
      return '';
